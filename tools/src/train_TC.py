    ...
    for epoch in range(args.num_train_epochs):
        ...
        with torch.no_grad():
            ...
            for iteration, (input_ids, attention_mask, labels) in tqdm(enumerate(dev_loader)):
                ...
                #         if labels[b, l] != -100:
                #             label_ids.append(TC_labels_inv[labels[b, l].item()])
                #             preds_list.append(TC_labels_inv[predictions[b, l].item()])
                for b in range(batch_size):
                    label_ids_temp = []
                    preds_list_temp = []
                    for l in range(max_seq_len):
                        if labels[b, l] != -100:
                            label_ids_temp.append(TC_labels_inv[labels[b, l].item()])
                            preds_list_temp.append(TC_labels_inv[predictions[b, l].item()])
...
def main():
    ...
    TC_labels = dict()
    TC_labels["0"] = -100
    TC_labels["Name_Calling,Labeling"] = 1
    TC_labels["Black-and-White_Fallacy"] = 2
    TC_labels["Loaded_Language"] = 3
    TC_labels["Exaggeration,Minimization"] = 4
    TC_labels["Exaggeration,Minimisation"] = 4
    TC_labels["Whataboutism,Straw_Men,Red_Herring"] = 5
    TC_labels["Appeal_to_Authority"] = 6
    TC_labels["Appeal_to_fear-prejudice"] = 7
    TC_labels["Doubt"] = 8
    TC_labels["Repetition"] = 9
    TC_labels["Slogans"] = 10
    TC_labels["Flag-Waving"] = 11
    TC_labels["Causal_Oversimplification"] = 12
    TC_labels["Bandwagon,Reductio_ad_hitlerum"] = 13
    TC_labels["Thought-terminating_Cliches"] = 0
    TC_labels_inv = {v: k for k, v in TC_labels.items()}

    input_examples = read_examples_from_file(args.data_file, TC_labels, args.random_n)
    train_data = convert_examples_to_features(input_examples, tokenizer, TC_labels, ignore_index, max_seq_len)

    valid_examples = read_examples_from_file(args.dev_file, TC_labels, args.random_n)
    dev_data = convert_examples_to_features(valid_examples, tokenizer, TC_labels, ignore_index, max_seq_len)

    # test_examples = read_examples_from_file(args.test_file, TC_labels, args.random_n)
    # test_data = convert_examples_to_features(test_examples, tokenizer, TC_labels, ignore_index, max_seq_len)
    ...
    train(model, train_loader, dev_loader, args, optimizer, device, scheduler, dev_loader, tokenizer, valid_examples,
          TC_labels_inv)
